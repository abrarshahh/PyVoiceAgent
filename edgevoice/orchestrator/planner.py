...
class PlannerAgent:
    ...
    async def create_plan(self, user_input: str, context: str = "") -> ExecutionPlan:
        if self.chain is None:
            raise NotImplementedError("LLM is not configured/implemented yet. Plan creation lands in Phase 1.")
...
if __name__ == "__main__":
    ...
    agent = PlannerAgent()
    try:
        plan = asyncio.run(agent.create_plan("Read requirements.txt and create a summary in summary.txt"))
        print(plan.model_dump_json(indent=2))
    except Exception as e:
        print(f"Error running standalone planner: {e}")
